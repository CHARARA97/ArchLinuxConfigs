...
var PreviewDrawingController = class {
  constructor(plugin, previewEl, view, file, options = {}) {
    ...
    this.toolbarMoveButton = null;
    this.toolbarIsDragging = false;
  ...
  scheduleResize() {
    if (this.resizeFrameId !== null || this.toolbarIsDragging) {
      return;
    }
  ...
  scheduleFloatingControlsPosition() {
    if (this.positionFrameId !== null || this.toolbarIsDragging) {
      return;
    }
  ...
  updateFloatingControlsPosition() {
    ...
    if (this.toolbarManualPosition) {
      ...
      const top = clamp(Math.round(this.toolbarManualPosition.top), viewportTop + 8, Math.max(viewportTop + 8, viewportTop + viewportHeight - toolbarRect.height - 8));
      const right = clamp(Math.round(this.toolbarManualPosition.right), 8, Math.max(8, viewportLeft + viewportWidth - 48));
      const props = {
        "--notedraw-toolbar-right": `${right}px`,
        "--notedraw-toolbar-top": `${top}px`,
        "--notedraw-palette-top": `${top + 42}px`,
        "--notedraw-text-panel-top": `${top + 42}px`
      };
  ...
  createToolbarMoveButton() {
    const button = this.toolbar.createEl("button", {
      cls: "notedraw-toolbar-move-button",
      attr: { type: "button", title: this.plugin.t("movePanel"), "aria-label": this.plugin.t("movePanel") }
    });
    (0, import_obsidian.setIcon)(button, "move");
    button.addEventListener("mousedown", (event) => event.preventDefault());
    button.addEventListener("pointerdown", (event) => this.startToolbarDrag(event));
    this.toolbarMoveButton = button;
  ...
  onToolbarDragMove(event) {
    ...
    event.preventDefault();
    event.stopPropagation();
    // 第一次移动时初始化偏移量和原始 CSS 位置
    if (this.toolbarDrag.first) {
      this.toolbarDrag.first = false;
      this.toolbarDrag.offsetY = event.clientY - this.toolbar.getBoundingClientRect().top;
      this.toolbarDrag.offsetX = event.clientX - this.toolbar.getBoundingClientRect().left;
      this.toolbarDrag.origTop = this.toolbar.getBoundingClientRect().top;
      this.toolbarDrag.origLeft = this.toolbar.getBoundingClientRect().left;
    }
    const toolbarRect = this.toolbar.getBoundingClientRect();
    const viewport = window.visualViewport;
    const viewportY = viewport?.offsetTop || 0;
    const viewportX = viewport?.offsetLeft || 0;
    const viewportH = Math.max(120, viewport?.height || window.innerHeight || 120);
    const viewportW = Math.max(160, viewport?.width || window.innerWidth || 160);
    // 目标位置 = 鼠标 - 偏移量
    let targetTop = event.clientY - this.toolbarDrag.offsetY;
    let targetLeft = event.clientX - this.toolbarDrag.offsetX;
    targetTop = clamp(targetTop, viewportY + 8, Math.max(viewportY + 8, viewportY + viewportH - toolbarRect.height - 8));
    targetLeft = clamp(targetLeft, viewportX + 8, Math.max(viewportX + 8, viewportX + viewportW - toolbarRect.width - 8));
    // 仅用 transform 移动工具栏，不设 toolbarManualPosition，
    // 让 updateFloatingControlsPosition 进正常分支（CSS 变量随便设，transform 覆盖）
    const dx = targetLeft - this.toolbarDrag.origLeft;
    const dy = targetTop - this.toolbarDrag.origTop;
    this.toolbar.style.transform = `translate(${Math.round(dx)}px, ${Math.round(dy)}px)`;
    // 调色板和文本面板通过原生 style.setProperty 同步跟随
    const topPx = `${Math.round(targetTop + 42)}px`;
    this.previewEl.style.setProperty('--notedraw-palette-top', topPx);
    this.previewEl.style.setProperty('--notedraw-text-panel-top', topPx);
  }
  ...
  stopToolbarDrag() {
    if (this.toolbar && this.toolbar.style.transform) {
      ...
      this.toolbarManualPosition = { top, right };
      // 用 style.top/style.right（内联样式）替代 CSS 变量，
      // 内联样式优先级最高，不会被 updateFloatingControlsPosition 的 setCssProps 覆盖
      this.toolbar.style.top = `${top}px`;
      this.toolbar.style.right = `${right}px`;
      // 调色板/文本面板通过 CSS 变量跟随
      const setVar = (el, name, val) => { if (el) el.style.setProperty(name, val); };
      setVar(this.previewEl, '--notedraw-palette-top', `${top + 42}px`);
      setVar(this.previewEl, '--notedraw-text-panel-top', `${top + 42}px`);
